...
export default function ExerciseDemo({
  ...
  return (
    <div className="rounded-3xl bg-[#0a0a0a] border border-neutral-800 overflow-hidden shadow-xl">
      ...
      <div className="p-4 border-b border-neutral-800 flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="text-xs font-bold text-amber-400">{targetSets} Sets</div>
          <div className="text-[11px] text-neutral-400">{targetReps} Reps</div>
        </div>
      </div>

      {/* Tabs */}
      <div className="flex border-b border-neutral-800 bg-[#050505]/60 text-xs font-bold">
        <button
          type="button"
          onClick={() => setActiveTab('visual')}
          className={`flex-1 py-2.5 text-center transition-colors flex items-center justify-center gap-1.5 ${
            activeTab === 'visual' ? 'text-red-400 border-b-2 border-red-500 bg-red-500/5' : 'text-neutral-400 hover:text-neutral-200'
          }`}
        >
        ...
        <button
          ...
          className={`flex-1 py-2.5 text-center transition-colors ${
            activeTab === 'instructions' ? 'text-amber-400 border-b-2 border-amber-400 bg-amber-500/5' : 'text-neutral-400 hover:text-neutral-200'
          }`}
        >
          Instructions
        </button>
        <button
          ...
          className={`flex-1 py-2.5 text-center transition-colors ${
            activeTab === 'safety' ? 'text-amber-400 border-b-2 border-amber-400 bg-amber-500/5' : 'text-neutral-400 hover:text-neutral-200'
          }`}
        >
          Form & Safety
        </button>
      ...
      <div className="p-4">
        {activeTab === 'visual' && (
          <div className="relative rounded-2xl bg-[#050505] border border-neutral-800/80 p-4 flex flex-col items-center justify-center overflow-hidden">
                ...
                <div className="flex items-center gap-1.5">
                  {viewMode === 'realistic' && (
                    <button
                      ...
                      className="px-2 py-1 rounded-lg bg-[#0a0a0a] border border-neutral-800 text-[10px] font-bold text-neutral-300 hover:text-white flex items-center gap-1 transition-colors"
                    >
                      {backdrop === 'light' ? (
                        <>
                  ...
                  <div className="flex items-center bg-[#0a0a0a] p-0.5 rounded-lg border border-neutral-800 text-[10px] font-bold">
                    <button
                      type="button"
                      onClick={() => setViewMode('realistic')}
                      className={`px-2 py-1 rounded transition-colors flex items-center gap-1 ${
                        viewMode === 'realistic' ? 'bg-red-500 text-white' : 'text-neutral-400 hover:text-white'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-2 py-1 rounded transition-colors flex items-center gap-1 ${
                        viewMode === 'vector' ? 'bg-neutral-800 text-white' : 'text-neutral-400 hover:text-white'
                      }`}
                    >
            ...
            {viewMode === 'realistic' && exerciseMedia && !imgError ? (
              <div
                className={`relative w-full max-w-[380px] h-[250px] rounded-2xl border shadow-2xl flex items-center justify-center overflow-hidden p-2 transition-all duration-300 ${
                  backdrop === 'light'
                    ? 'bg-gradient-to-b from-white via-neutral-100 to-neutral-200 border-neutral-300 shadow-neutral-950/20'
                    : 'bg-[#050505] border-neutral-800 shadow-black'
                }`}
              >
                ...
                <div
                  className={`absolute bottom-2 left-2 px-2 py-0.5 rounded text-[9px] font-mono border backdrop-blur-sm ${
                    backdrop === 'light'
                      ? 'bg-white/80 border-neutral-200 text-neutral-700 shadow-sm'
                      : 'bg-[#0a0a0a]/80 border-neutral-800 text-neutral-400'
                  }`}
                >
            ...
            <div className="mt-4 flex items-center justify-between w-full pt-3 border-t border-neutral-800/60">
              <button
                type="button"
                onClick={() => setIsPlaying(!isPlaying)}
                className="px-3.5 py-1.5 rounded-xl bg-neutral-900 border border-neutral-800 hover:border-amber-500/40 text-xs font-bold text-white flex items-center gap-1.5 transition-all shadow-md active:scale-95"
              >
              ...
              <div className="flex items-center gap-2">
                ...
                {secondaryMuscles.slice(0, 2).map((m, idx) => (
                  <div key={idx} className="hidden sm:flex items-center gap-1 text-[10px] font-medium text-neutral-400 bg-neutral-900 px-2 py-1 rounded-lg border border-neutral-800">
          ...
          <div className="space-y-2.5 py-2">
            {instructions.length > 0 ? (
              instructions.map((step, idx) => (
                <div key={idx} className="flex items-start gap-3 p-2.5 rounded-xl bg-[#050505]/60 border border-neutral-800/80 text-xs text-neutral-300">
                  <span className="w-5 h-5 rounded-full bg-amber-500/20 text-amber-400 border border-amber-500/30 font-bold flex items-center justify-center flex-shrink-0 text-[10px]">
                    {idx + 1}
                  </span>
              ...
              <p className="text-xs text-neutral-400">Perform exercise with steady control and continuous core engagement.</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'safety' && (
          <div className="space-y-2.5 py-2">
            {safetyTips.length > 0 ? (
              ...
              <div className="flex items-start gap-2.5 p-2.5 rounded-xl bg-neutral-900 border border-neutral-800 text-xs text-neutral-300">
